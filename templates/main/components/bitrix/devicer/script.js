...
BX.saleOrderAjax = { // bad solution, actually, a singleton at the page
	...
	editDeliveryInfo: function(deliveryNode) {
		...
		var deliveryInfoContainer = deliveryNode.querySelector('delivery_main_title');
	...
	//используем коллекцию, инициализированную в предыдущем методе
		...
		while (group = groupIterator())
		{
			...
			{
				if (property.getGroupId()==2) { //если это свойство является параметром доставки
...
// custom code for add personal data

var siteCookie = {
	set: function set(name, value, props) {
		props = props || {};
...
function btnNextHandler(e) {
	...
	u_name = window.qs('#soa-property-1').value;
	u_email = window.qs('#soa-property-2').value;
	u_phone = window.qs('#soa-property-3').value;
	u_city = '';
	// u_city = window.qs('#soa-property-5').value;

	if (u_name != '' && u_email != '' /*&& u_phone != ''*/) {
		var xhr = new XMLHttpRequest();
		var body = "name=" + encodeURIComponent(u_name) + "&email=" + encodeURIComponent(u_email) + "&phone=" + encodeURIComponent(u_phone) + "&city=" + encodeURIComponent(u_city);
		xhr.open("POST", '/ajax/personal_order.php', true);
		xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
		xhr.send(body);
		xhr.onreadystatechange = function() {
			// console.log(xhr.responseText);
			if (xhr.responseText != 'error') {
				delivery_block = window.qs('#bx-soa-order-form .pandd');
				order_block = window.qs('#bx-soa-orderSave');
				delivery_block.classList.add('opened');
				order_block.classList.add('opened');

				btn_next_step2.style.display = 'none';
			}
		}
	}
}
...
document.addEventListener('DOMContentLoaded', function () {
	btn_next_step2 = document.querySelector(".btn_next_step_order");
	btn_next_step2.addEventListener('click', btnNextHandler);
});
