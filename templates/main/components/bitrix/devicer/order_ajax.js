...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		editActivePaySystemBlock: function(activeNodeMode)
		{
			...
			// Добавили блок комментария к заказу
		...
		editFadePaySystemBlock: function()
		{
			...
			// Добавили блок комментария к заказу
			this.editPropsComment(paySystemBlockNode);
		},
		...
		editDeliveryInfo: function(deliveryNode)
		{
			...
			var deliveryItemsContainer = BX.create('DIV', {props: {className: 'col-sm-12 bx-soa-delivery'}}),
				instaPropertyContainer = BX.create('DIV', {props: {className: 'col-sm-12 bx-soa-insta'}}),
				group, property, groupIterator = this.propertyCollection.getGroupIterator(), propsIterator, instaContainer, isDeliveryItemsContainerExists, isInstaPropertyContainerExists;

			instaContainer = document.getElementById('bx-soa-coupon');
			//console.log(instaContainer);

			isDeliveryItemsContainerExists = document.querySelector('.col-sm-12.bx-soa-delivery');
			isInstaPropertyContainerExists = document.querySelector('.col-sm-12.bx-soa-insta');

			console.log(isDeliveryItemsContainerExists);
			console.log(isInstaPropertyContainerExists);

			if (!deliveryItemsContainer)
				deliveryItemsContainer = this.propsBlockNode.querySelector('.col-sm-12.bx-soa-delivery');

			if (!instaPropertyContainer)
				instaPropertyContainer = this.propsBlockNode.querySelector('.col-sm-12.bx-soa-insta');
			...
			while (group = groupIterator())
			{
				...
				{
					//Если свойство совпадает с названием поля адреса в вашей системе
					if (property.getId() == 7 && isDeliveryItemsContainerExists == null) {
						//вставляем свойство в подготовленный контейнер
						this.getPropertyRowNode(property, deliveryItemsContainer, false);
						//контейнер вместе со свойством в нём добавляем в конце блока с описанием (deliveryInfoContainer)
						dNodeCopy.appendChild(deliveryItemsContainer);

					}
					if (property.getId() == 20 && isInstaPropertyContainerExists == null) {
						this.getPropertyRowNode(property, instaPropertyContainer, false);
						instaContainer.appendChild(instaPropertyContainer);
